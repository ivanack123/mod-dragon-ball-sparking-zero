# Instalador GRAFICO del mod de accesibilidad para DRAGON BALL Sparking ZERO
#
# Asistente con ventanas y botones: Bienvenida, Carpeta, Instalando, Finalizar.
# Se compila a Instalar.exe con PS2EXE usando -noConsole.
#
# ACCESIBILIDAD (es lo que mas importa aqui, el usuario es ciego):
#  - Controles estandar de Windows Forms, que NVDA lee de forma nativa.
#  - Cada campo lleva su etiqueta y su AccessibleName y AccessibleDescription.
#  - Enter activa el boton principal y Escape cancela.
#  - Al cambiar de paso, el titulo de la ventana cambia Y el foco salta al
#    control principal, que es como NVDA se entera de que hubo un cambio.
#  - Nada de temporizadores que muevan el foco solos ni animaciones.
...
# Campo de la ruta (solo visible en el paso 2)
...
function Ir-A { param([int]$n)
    ...
    switch ($n) {
        1 {
            $form.Text = 'Instalador del mod de accesibilidad - Bienvenida'
            $titulo.Text = 'Mod de accesibilidad para DRAGON BALL Sparking ZERO'
            $texto.Text = "Este asistente instalara el mod que hace hablar al juego, para poder jugarlo sin ver.`r`n`r`nLee los menus, los subtitulos, la vida y el ki en combate, las recompensas, la Enciclopedia, la Tienda y mucho mas.`r`n`r`nAntes de continuar, cierra el juego si lo tienes abierto.`r`n`r`nPulsa Siguiente para elegir la carpeta del juego."
            $btnPrincipal.Text = 'Siguiente'
        ...
        3 {
            ...
            $texto.Text = 'Copiando los archivos del mod. Espera un momento, por favor.'
            $barra.Visible = $true
        ...
        4 {
            ...
            $texto.Text = "El mod ha quedado instalado correctamente.`r`n`r`nYa puedes abrir el juego. Deberia empezar a hablarte en cuanto llegue a la pantalla de inicio.`r`n`r`nSi algun dia quieres quitarlo, borra el archivo dwmapi.dll de la carpeta del juego.`r`n`r`nPulsa Finalizar para cerrar."
    ...
    switch ($script:paso) {
        ...
        2 {
            ...
            if (-not (Test-Path $ORIGEN)) {
                [System.Windows.Forms.MessageBox]::Show('No encuentro la carpeta "archivos" junto al instalador. Descomprime el paquete entero antes de ejecutarlo.', 'Faltan archivos', 'OK', 'Error') | Out-Null
                return
            }
            $script:destino = $ruta
            Ir-A 3
